...
/**
  \fn           int32_t genx320_Uninit(CAMERA_SENSOR_DEVICE *cpi_genx320_camera_sensor,
                                       CAMERA_SENSOR_SLAVE_I2C_CONFIG *i2c_cfg)
  \brief        Un-initialize GENX320 Camera Sensor.
  \param[in]    none
  \return       \ref execution_status
  */
...
{  
/*    
    // SPG: ensure streaming mode is active
	psee_PM3C_config();

	// Configure CPI SPG to output 8-bit increment pattern at max speed
	//psee_sensor_write(REG_CPI_PKT_SELF_TEST, 0x00000002);  // Optional: basic enable first
	psee_sensor_write(REG_CPI_PKT_SELF_TEST,
		(0x0 << 16) |  // self_test_repeat = 0 (no repetition)
		(0x1 << 3)  |  // self_test_rate = 0 (max speed: 1 event per cycle)
		(0x2 << 0));   // self_test_ctrl = 2 (increment mode)
*/
