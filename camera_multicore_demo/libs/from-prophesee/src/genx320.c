...
static void execute_reg_op_sequence(const struct sequence seq) {
	for(unsigned int i = 0; i < seq.len; i++) {
		if(seq.ops[i].op == WRITE) {
			#ifdef DEBUG
				printf("Writing to register 0x%04X with data 0x%08lX\r\n", seq.ops[i].args.write.addr, seq.ops[i].args.write.data);
			#endif
			...
			uint32_t data = 0;
			#ifdef DEBUG
				printf("Reading from register 0x%04X\r\n", seq.ops[i].args.write.addr);
			#endif
			...
			#ifdef DEBUG
				printf("Delay for %lu ms\r\n", delay_ms);
			#endif
			psee_sleep_ms_imp(delay_ms);
		}
		#ifdef DEBUG
			printf("Waiting for 1 ms...\r\n");
		#endif
		psee_sleep_ms_imp(1);
	}
}
...
const struct issd *psee_open_evt() {
   ...
   if ((chip_id != SAPHIR_MP_ID) && (chip_id != SAPHIR_ES_ID)) {
       while(1){
		// Print chip ID error every two seconds
		...
		#ifdef DEBUG
			printf("Sensor ID error, should be %lX or %lx, is  %lX \n\r", SAPHIR_MP_ID,SAPHIR_ES_ID , chip_id);
		#endif
   	...
   	if(chip_id == SAPHIR_ES_ID) genx320_default_biases = genx320es_default_biases;

	else genx320_default_biases = genx320mp_default_biases;

	/* Set Quite Biases */
	//genx320_default_biases = quiet_biases;
	...
	// Optional: print the internal sensor clock tree at runtime
	clk_freq_dict clk = get_sensor_clk_freq(10);  // 10 MHz input clock

	printf("========= GenX320 Clock Tree =========\n\r");
	printf("Input Clock (in_freq):        %lu MHz\n\r", clk.in_freq);
	printf("System Clock (sys_freq):      %lu MHz\n\r", clk.sys_freq);
	printf("Event Core Clock (evt_icn):   %lu MHz\n\r", clk.evt_icn_freq);
	printf("RISC-V CPU Clock (cpu_ss):    %lu MHz\n\r", clk.cpu_ss_freq);
	printf("PLL Input Clock:              %lu MHz\n\r", clk.pll_in_freq);
	printf("PLL VCO Output:               %lu MHz\n\r", clk.pll_vco_freq);
	printf("PHY Clock (pre-divider):      %lu MHz\n\r", clk.phy_freq);
	printf("MIPI Clock (post-ODF):        %lu MHz\n\r", clk.mipi_freq);
	printf("MIPI Escape Clock:            %lu MHz\n\r", clk.esc_freq);
	printf("======================================\n\r");


	// Set divider to 2: shift 2 << 2 = 0x08
	// Enable divider: bit 0 = 1
	// Select divider: bit 1 = 1
	// Final value: 0x08 | 0x03 = 0x0B
	//psee_sensor_write(0x0208, 0x0000000B); // Set CPU clock to CLKI / 2 = 5 MHz
   // Define the clock division factor

   /*
	#define CPU_SS_CLK_DIV 2  // Resulting clock = 10 MHz / 2 = 5 MHz

	// Step 1: Build the register value
	uint32_t cpu_ss_clk_ctrl = 0;
	cpu_ss_clk_ctrl |= (1 << 0);               // Enable divider
	cpu_ss_clk_ctrl |= (1 << 1);               // Select divided clock
	cpu_ss_clk_ctrl |= (CPU_SS_CLK_DIV << 2);  // Set division factor

	uint32_t sys_clk_ctrl;
	psee_sensor_read(0x0204, &sys_clk_ctrl);
	sys_clk_ctrl &= ~(1 << 15); // Clear auto mode
	psee_sensor_write(0x0204, sys_clk_ctrl);
	
	// Step 2: Write to the CPU_SS clock control register
	psee_sensor_write(0x0208, cpu_ss_clk_ctrl);

	// evt_icn_clk: enable divider, set divider to 2, select divided input
	// psee_sensor_write(0x0210, 0x0000000B); // 0b00000000000000000000000000001011
	*/
	
	// Optional: print the internal sensor clock tree at runtime
	clk = get_sensor_clk_freq(10);  // 10 MHz input clock
...
const BIAS_Params_t quiet_biases = {
	.pr       		= 61,
	.fo         	= 19,
	.fes        	= 63,
	.hpf        	= 0,
...
// Don't use this function the dropped events are not evenly distributed so you get verwierd output!
void psee_sensor_configure_throttle(void)
{
    // 1. Enable ERC with drop-on-backpressure
    psee_sensor_write(ERC_PIPELINE_CONTROL,
                      ERC_PIPELINE_CONTROL_ENABLE |
                      ERC_PIPELINE_CONTROL_DROP_NBACKPRESSURE);

    // 2. Set ERC reference period to 200 µs
    psee_sensor_write(0x00006020, 200);

    // 3. Configure temporal drop to target ~1.3Mev/s
    // Dij = 0x104 = 360 (Q1.9), for 200 µs → ~1.8 Mev/s (but test shows it's ~1.3Mev/s)
    // Disable 'drop all above threshold' by setting drop_all_td_when_drop_geq = 0x201 (disabled)
    uint32_t dij = 0x10A;
    uint32_t drop_all_disable = 0x201 << 4;
    uint32_t t_dropping_en = 1 << 2;

    psee_sensor_write(0x00006014, t_dropping_en | drop_all_disable | dij);
}
...
void psee_sensor_set_CPI_EVT20() {
	...
	/* Configure Packet and Frame sizes */
  	psee_sensor_write(CPI_FRAME_SIZE_CONTROL, 	0x40UL);											/* Frame Size in Packets : 64 Packets */
	...
	// psee_sensor_write(0x8014UL, 0x00004E20); // 0x4E20 = 20,000 cycles = 1 ms @ 20 MHz

	psee_sensor_set_deadtime(deadtime_between_frames);											
}


/**
  * @brief Function to set the deadtime between the frames
  * @param deadtime deadtime in microseconds
*/
void psee_sensor_set_deadtime(uint32_t deadtime) {
	uint32_t delay_in_cycles = deadtime * 10; // Convert to microseconds
	psee_sensor_write(0x8014UL, delay_in_cycles); // 875 µs frame blanking at 10 MHz
}
