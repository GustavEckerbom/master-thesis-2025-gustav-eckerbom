...
void BOARD_Pinmux_Init(){
	...
	ARM_DRIVER_GPIO *BOARD_CAM_ULP_RST_GPIO_GPIOdrv = &ARM_Driver_GPIO_(BOARD_CAM_ULP_RSTN_GPIO_PORT);
	...
	// Init the LED GPIOs and header IOs
	END_OF_FRAMEdrv->Initialize(END_OF_FRAME_GPIO_PIN, NULL);
	END_OF_FRAMEdrv->PowerControl(END_OF_FRAME_GPIO_PIN, ARM_POWER_FULL);
	END_OF_FRAMEdrv->SetDirection(END_OF_FRAME_GPIO_PIN, GPIO_PIN_DIRECTION_OUTPUT);
	END_OF_FRAMEdrv->SetValue(END_OF_FRAME_GPIO_PIN, GPIO_PIN_OUTPUT_STATE_LOW);
	...
	uint32_t config_button =
        PADCTRL_READ_ENABLE |
        PADCTRL_SCHMITT_TRIGGER_ENABLE| PADCTRL_DRIVER_DISABLED_PULL_UP | PADCTRL_DRIVER_OPEN_DRAIN;


    uint32_t arg = ARM_GPIO_IRQ_POLARITY_LOW | ARM_GPIO_IRQ_SENSITIVE_EDGE;

    BOARD_USER_BUTTON_GPIO_GPIOdrv->Initialize(BOARD_USER_PUSH_GPIO_PIN, NULL);
    ...
    pinconf_set(PORT_(BOARD_USER_PUSH_GPIO_PORT), BOARD_USER_PUSH_GPIO_PIN, PINMUX_ALTERNATE_FUNCTION_0, config_button);
    BOARD_USER_BUTTON_GPIO_GPIOdrv->Initialize(BOARD_USER_PUSH_GPIO_PIN, button_irq_callback);
    BOARD_USER_BUTTON_GPIO_GPIOdrv->Control(BOARD_USER_PUSH_GPIO_PIN, ARM_GPIO_ENABLE_INTERRUPT, &arg);
